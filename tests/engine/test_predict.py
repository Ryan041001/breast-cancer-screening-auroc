...
def test_load_model_from_checkpoint_restores_saved_transformer_fusion_config(
    tmp_path: Path,
) -> None:
    ...
    assert isinstance(loaded_model, PairedBreastModel)
    assert loaded_model.fusion_head_config.variant == "transformer"
    assert loaded_model.fusion_head_config.hidden_dim == 256
    assert loaded_model.fusion_head_config.transformer_layers == 2
    assert loaded_model.fusion_head_config.transformer_heads == 4
